...
This file is loaded into every Claude Code session in this repo. Keep it tight; high-signal notes only.
...
The user (Corey) makes product decisions; Claude does the implementation. Keep code tight and avoid over-engineering — every milestone earns the next.
...
- M5: ✅ three-state sidebar (`full` / `compact 52pt` / `hidden`, ⌘⌃S cycles), top chrome strip with explicit `WindowDragHandle` (`window.isMovable = false` globally so tab DnD wins), View menu becomes navigation hub, Help + Debug menus, custom About panel sourced from `PebbleApp` constants, declarative menu DSL. URL ⌘+click + mouse shape + `⌘=`/`⌘-`/`⌘0` font size + `⌘K` Clear Pane + sidebar mode persistence. Tab + workspace manual rename via right-click → *Rename…* popover (empty input clears back to cwd); rename `TextField` lives in `PebbleRenameField` so both call sites share it. OSC 133 / FinalTerm command-status — small red dot per tab pill on non-zero exit, hover for `exit N · 12.4s`, `⌘↑`/`⌘↓` jump-to-prompt, ZDOTDIR wrapper installs the hooks without touching the user's `.zshrc`. Failure dot lifted up to the workspace row (precedence: attention > failure > running > idle), three sidebar reads folded into a single `sidebarReadout` DFS, `Theme.activityRunning/Failure/Attention` the single source for the dot palette. `.app` bundle pipeline: `scripts/build-app.sh` + `scripts/build-dmg.sh` produce `Pebble.app` and a drag-to-Applications DMG; `Theme.swift` got a custom bundle resolver replacing SPM's `.module` accessor (which `fatalError`s in `.app` context). AppIcon (`branding/AppIcon.png` — cyber-minimalist `[ - · ]` on charcoal squircle, sized into a full `.iconset` via sips at build time). macOS 14 (Sonoma) is the floor — `@Observable` requires it. Find in scrollback (`⌘F`) per-tab via libghostty's `start_search` / `search:<text>` / `navigate_search:next|previous` / `end_search` named actions; the search bar overlays the active pane's top-right and tracks per-`Session` state so multiple panes can search simultaneously. Settings UI still pending; Apple Developer ID + notarization deferred until the project has real users.
